// Enhanced script.js with recipe category integration
...
let pendingRecipeData = null; // For recipe category modal
...
function handleURLParameters() {
  ...
  if (hash === 'shopping') {
    ...
    // Check if we need to open a specific recipe
    const openRecipeId = localStorage.getItem('openRecipeId');
    if (openRecipeId) {
  ...
  recipesList.innerHTML = recipes.map(recipe => `
    <div class="recipe-card" onclick="openRecipeViewer('${recipe.id}')">
      <div class="recipe-image">
        ...
        ${recipe.category ? `
          <div class="recipe-category-badge ${recipe.category}">
            <i class="fas ${getCategoryIcon(recipe.category)}"></i>
          </div>
        ` : ''}
      </div>
...
function getCategoryIcon(category) {
  switch (category) {
    case 'healthy': return 'fa-leaf';
    case 'unhealthy': return 'fa-hamburger';
    case 'dessert': return 'fa-ice-cream';
...
function removeRecipeImage(index) {
  if (currentRecipe && currentRecipe.images) {
    currentRecipe.images.splice(index, 1);
    renderRecipeImages(currentRecipe.images);
  }
}
...
async function saveRecipe() {
  ...
  const recipeData = {
    id: currentRecipe?.id || generateId(),
    title,
    description,
    ingredients,
    method,
    images: currentRecipe?.images || [],
    createdAt: currentRecipe?.createdAt || Date.now(),
    updatedAt: Date.now()
  };
  
  // Store recipe data for category modal
  pendingRecipeData = recipeData;
  
  // Show category selection modal
  showRecipeCategoryModal();
}

function showRecipeCategoryModal() {
...
function hideRecipeCategoryModal() {
  const modal = document.getElementById("recipeCategoryModal");
  if (modal) {
...
async function selectRecipeCategory(category) {
  if (!pendingRecipeData) return;
  
  // Add category to recipe data
  pendingRecipeData.category = category;
  
  try {
    // Save to global recipes
    const existingIndex = recipes.findIndex(r => r.id === pendingRecipeData.id);
    if (existingIndex >= 0) {
      recipes[existingIndex] = pendingRecipeData;
    } else {
      recipes.push(pendingRecipeData);
    }
    
    // Save to Firebase and localStorage
    await saveRecipesToStorage();
    
    hideRecipeCategoryModal();
    showRecipesPage();
    showToast(`Recipe saved as ${category}!`, "success");
...
function setupRecipeEventListeners() {
  ...
  if (editRecipeBtn) editRecipeBtn.addEventListener("click", () => openRecipeEditor(currentRecipe));
  ...
  if (addRecipeImageBtn) addRecipeImageBtn.addEventListener("click", () => recipeImageUpload.click());
  
  if (recipeImageUpload) {
    recipeImageUpload.addEventListener("change", handleRecipeImageUpload);
  }
}
...
function setupModalEventListeners() {
  ...
  if (closeCategoryModal) {
    closeCategoryModal.addEventListener("click", hideRecipeCategoryModal);
  }
  ...
  Array.from(files).forEach(file => {
    if (file.type.startsWith('image/')) {
      ...
      reader.onload = (e) => {
        ...
        currentRecipe.images.push(e.target.result);
        renderRecipeImages(currentRecipe.images);
      };
...
window.openRecipeViewer = (recipeId) => {
  const recipe = recipes.find(r => r.id === recipeId);
  if (recipe) openRecipeViewer(recipe);
};
...
function setupBasicEventListeners() {
  ...
  // This is a placeholder to maintain the existing functionality
